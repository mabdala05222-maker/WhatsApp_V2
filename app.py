...
# تحديث تلقائي ذكي وصامت كل ثانيتين لجلب الرسائل الجديدة فوراً
st_autorefresh(interval=2000, key="whatsapp_global_refresh")

# 2. هندسة الـ CSS لتحويل الواجهة بالكامل إلى WhatsApp Web (Dark/Light Hybrid)
st.markdown("""
    <style>
    /* خلفية الشات الشهيرة لواتساب */
    ...
    /* تصميم السايدبار الأيسر الداكن المتطابق مع واتساب */
...
# 3. اتصالات الداتابيز المستقرة وقفل الـ Threading لمنع الـ Crash
...
# إعداد المتغيرات بداخل الـ Session State
            ...
            if st.button("إنشاء الحساب والولوج فوراً", use_container_width=True):
                if reg_user and reg_pass:
                    try:
                        ...
                        conn.commit()
                        # الدخول الفوري والمباشر دون رجوع
                        st.session_state['logged_in_user'] = reg_user
                        st.rerun()
                    except sqlite3.IntegrityError:
                        st.error("❌ اسم المستخدم هذا محجوز ومسجل بالفعل!")
                else:
                    st.warning("⚠️ برجاء إدخال البيانات كاملة أولاً.")
        st.markdown("</div>", unsafe_allow_html=True)

# ==========================================
# الشاشة الثانية: واجهة المحادثات والـ Chat الحقيقي
# ==========================================
else:
    my_name = st.session_state['logged_in_user'].strip()
    
    # أ) الشريط الجانبي (السايدبار)
    ...
    # أزرار التحكم بالحساب الإحترافية
    ...
    # جلب جميع المستخدمين المسجلين ما عدا أنا
    ...
    if all_users:
        ...
        st.sidebar.markdown("<p style='color:#8696a0; padding-right:10px; font-size:13px;'>لا توجد جهات اتصال أخرى مسجلة.</p>", unsafe_allow_html=True)

    # ب) نافذة الدردشة الرئيسية (اليمنى)
    ...
    if active_chat:
        ...
        # تحديث الرسائل المستلمة لتصبح مقروءة صح
        c.execute("UPDATE messages SET is_read = 1 WHERE sender = ? AND receiver = ?", (active_chat, my_name))
        conn.commit()
        
        # جلب وعرض الرسائل المتبادلة بين الطرفين فقط بكل دقة
        ...
        for msg_id, sender, text, f_path, f_type, tm, is_read in chat_messages:
            ...
            with st.markdown('<div class="msg-container">', unsafe_allow_html=True):
                ...
                # عرض الميديا إن وجدت وبشكل نظيف
        ...
        # 3. منطقة الإدخال الاحترافية الحديثة (بدون نماذج وبدون أخطاء)
        # رفع الملف التلقائي (الميديا اختياري) فوق صندوق الكتابة مباشرة
        uploaded_file = st.file_uploader("📎 أرفق صورة / صوت / ملف للدردشة:", type=["png", "jpg", "jpeg", "pdf", "docx", "txt", "mp3", "wav", "m4a"], label_visibility="collapsed")
        
        # صندوق إدخال الشات الذكي من Streamlit (يرسل بمجرد ضغط Enter أو السهم المطور)
        txt_input = st.chat_input("اكتب رسالة هنا...")
        
        if txt_input or uploaded_file:
            # نتحقق لضمان عدم إرسال ضغطات فارغة
            ...
            if txt_stripped or uploaded_file:
                ...
                # حفظ الملف المرفق إذا وجد وتحديد نوعه
                ...
                # إدخال البيانات المكتوبة في الداتابيز فوراً
                c.execute("""
        ...
        # الشاشة الافتراضية الترحيبية للواتساب
        st.markdown("""
            <div style='text-align: center; margin-top: 15%; color: #667781;'>
                ...
                <p style='font-size: 13px; color: #8696a0; margin-top: 60px;'>🔒 جميع محادثاتك آمنة ومحفوظة محلياً بالكامل.</p>
            </div>
        """, unsafe_allow_html=True)
